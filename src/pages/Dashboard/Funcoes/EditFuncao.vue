...
onMounted(async () => {
  ...
  try {
    ...
    const { data } = await getFuncao(route.params.id as string)
...
async function handleSubmit() {
  ...
  try {
    ...
    await patchFuncao(payload, route.params.id as string)
...
<template>
  <section class="edit-funcao">
    <div class="edit-funcao__heading">
      <button
        type="button"
        class="edit-funcao__back"
        aria-label="Voltar para Funções"
        @click="goBack"
      >
        <img :src="iconChevronLeft" width="24" height="24" alt="" />
      </button>
      <h2 class="edit-funcao__title dashboard_title">EDITAR FUNÇÃO</h2>
    </div>

    <div v-if="fetching" class="edit-funcao__loading">
      <p>Carregando…</p>
    </div>

    <div v-else class="edit-funcao__panel">
      <form class="edit-funcao__form" @submit.prevent="handleSubmit">
        <div v-if="isAdmin" class="edit-funcao__field">
          <span class="edit-funcao__label night-field-label" id="empresa-label">EMPRESA*</span>
          <div ref="empresaFilterRef" class="edit-funcao__select">
            <button
              type="button"
            ...
            >
              ...
              <img
                ...
                :class="{ 'edit-funcao__select-chevron--open': empresaOpen }"
                :src="iconChevronDown"
                width="16"
                height="9"
                alt=""
            ...
            <ul
              v-if="empresaOpen"
              class="edit-funcao__select-menu"
              role="listbox"
              aria-labelledby="empresa-label"
            >
              <li>
        ...
        <div class="edit-funcao__field">
          <label class="edit-funcao__label night-field-label" for="nome">NOME*</label>
          <input
            id="nome"
            v-model="form.nome"
            type="text"
            class="edit-funcao__input"
        ...
        <div class="edit-funcao__field">
          <label class="edit-funcao__label night-field-label" for="descricao">DESCRIÇÃO</label>
          <textarea
            id="descricao"
            v-model="form.descricao"
            class="edit-funcao__textarea"
            placeholder="Descrição da função (opcional)"
            rows="4"
          />
        </div>

        <div class="edit-funcao__actions">
          <button type="button" class="edit-funcao__cancel" @click="goBack">
            CANCELAR
          </button>
          <button type="submit" class="edit-funcao__submit" :disabled="loading">
